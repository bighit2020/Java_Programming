...
public class ScoreServer {
	public static HashMap<String,String> calcScore(List<Student> list){
	//1.로컬변수 선언
	Student st=null;
	...
	for(int i=0;i<length;i++){
		if(name[i]!=null){
			...
			String res=Integer.toString(score[i])+","+
					Float.toString(((float)score[i]/subNum[i]));
			sMap.put(name[i],res );
			
			System.out.println(name[i]+","+ res);
		}
	  /* if(name[i]!=null){

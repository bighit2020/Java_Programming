...
public class ProdDAO {
	    ...
	    public void addProdInfo(ProdTO prodInfo){
	    	String code=null;
	    	String name= null;
	    	String color=null;
	    	int 	qty=0;
	    	
	    	 code=prodInfo.getProdCode();
	    	 name=prodInfo.getProdName();
		...
		public ArrayList<ProdTO> listProdInfo(){   
		  ...
		  try{			
				...
				ResultSet rs = stmt.executeQuery( query);	//쿼리를 실행한 후 리턴되는 레코드 셋을 rs에 저장한다.		 
				 ...
				 while( rs.next() ){				        //rs의 레코드셋을 변수에 할당한다.
					...
					ProdTO prodData= new ProdTO();     //TO객체를 생성한다.
